...
# --- FUNCȚIE AJUTĂTOARE PENTRU CURĂȚARE ---
def get_clean_env(var_name):
    """Citește variabila și elimină ghilimelele duble sau simple dacă există."""
    val = os.getenv(var_name)
    if val:
        # Eliminăm spațiile, apoi ghilimelele duble, apoi cele simple
        return val.strip().strip('"').strip("'")
    return None
# ------------------------------------------

# Folosim funcția de curățare pentru a prelua credențialele
...
# Cache-ul îl luăm brut momentan, îl procesăm special mai jos
CACHE_CONTENT = os.getenv("SPOTIPY_CACHE") 

SCOPE = "user-top-read user-read-recently-played"

def authentication():
    # Debugging pentru a confirma că ID-ul e curat
    # print(f"DEBUG ID (fără ghilimele): |{CLIENT_ID}|") 
    ...
    if CACHE_CONTENT:
        # Curățare specifică pentru JSON (care e mai complex)
        ...
        try:
            ...
            # Fallback scriere brută
            with open(cache_path, "w") as f:
                f.write(clean_content)

    # Inițializare Spotipy
    try:
        user = spotipy.Spotify(auth_manager=SpotifyOAuth(
            client_id=CLIENT_ID,        # Acum este curat (fără ghilimele)
            client_secret=CLIENT_SECRET,# Acum este curat
            redirect_uri=REDIRECT_URI,  # Acum este curat
            scope=SCOPE,
